...
image = cv2.imread('images/group definition_abstract algebra.png')
...
def create_text(text, x, y, size, font, pil_im, canvas):
...
async def on_ready():
    ...
    cv2.namedWindow("mister red", cv2.WINDOW_AUTOSIZE)
...
async def save_image(ctx: commands.Context):
    ...
    filename = ctx.message.content.removeprefix("!addnote ")
    ...
    for attachment in attachments:
        ...
        if any(attachment.filename.lower().endswith(ext) for ext in (".png", ".jpg", ".jpeg", ".gif", ".bmp", ".webp")):
            ...
            try:
                ...
                await ctx.send(f"❌ Failed to save `{filename}`: {e}")
        else:
...
async def slideshow():
    ...
    if image is None:
        ...
        create_text(f"{parts[0]}", -1, 20, 36, r"fonts\interb.ttf", pil_im, canvas)

        create_text(ltime, -2, 445, 22, r"fonts\interl.ttf", pil_im, canvas)
        create_text(temp, -3, 445, 22, r"fonts\interl.ttf", pil_im, canvas)
        # create_text("1 of 9 pages", 10, 450, 18, r"fonts\interl.ttf", pil_im, canvas)
